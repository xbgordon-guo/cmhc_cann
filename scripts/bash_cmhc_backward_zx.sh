...
rm -rf build/binary/ascend910b/src/mhc_pre_cmhc_backward
bash build.sh --opkernel --soc=ascend910b --ops=mhc_pre_cmhc_backward

# === Python Binding（改 binding/ 下代码后） ===
cd "${CMHC_CANN_DIR}/cmhc_backward/binding"
rm -rf build/ *.so *.o
...
SO_NAME=$(ls mhc_pre_cmhc_backward_torch_ops.cpython-311-*.so | head -1)
python -c "import torch; torch.ops.load_library('${SO_NAME}'); print('OK')"

# 精度验证
export ASCEND_RT_VISIBLE_DEVICES=1
python "${CANNBOT_SKILLS_DIR}/plugins-community/tilelang2ascendc-ops-generator/skills/tilelang2ascend-translator/scripts/verification_ascendc.py" "${CMHC_CANN_DIR}/cmhc_backward/"

# 性能验证
python "${CANNBOT_SKILLS_DIR}/ops/ops-profiling/scripts/msprof_perf_summary.py" --quick --output-dir="${CMHC_CANN_DIR}/cmhc_backward/"
