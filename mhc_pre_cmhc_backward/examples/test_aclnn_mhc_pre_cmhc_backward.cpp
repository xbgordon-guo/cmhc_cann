 ...
 * \brief aclnn test example for MhcPreCmhcBackward operator.
 *
 * MhcPreCmhcBackward is the backward operator of MhcPreCmhc. It consumes the forward
 * cached intermediates (h_pre, hc_before_norm, inv_rms) plus the pre-computed Birkhoff
 * permutation matrices (perm_mats, shape [n!, n, n]) and returns grad_x / grad_phi /
 * grad_alpha / grad_bias.
 *
 * Build & run through: bash build.sh --run_example mhc_pre_cmhc_backward eager cust ...
 */
...
// Build an aclTensor view with computed strides (row-major / C-contiguous).
static void MakeStrides(const std::vector<int64_t> &shape, std::vector<int64_t> &strides)
{
                            ...
                            aclTensor *&tensor)
{
    ...
    // Proper float32 -> bfloat16 conversion: round-to-nearest-even via bit truncation.
    for (int64_t i = 0; i < size; ++i) {
        uint32_t int_bits = 0;
        std::memcpy(&int_bits, &host_data[i], sizeof(uint32_t));
        uint32_t rounding_bias = int_bits & 0x0000ffffu; // capture low 16 bits for rounding
        int_bits >>= 16;
        if (rounding_bias > 0x00007fffu) {
            int_bits += 1; // round to nearest even
        }
        host_data_bf16[i] = static_cast<uint16_t>(int_bits);
    }
...
struct Tensors {
    ...
    void *h_pre_addr = nullptr, *hc_before_norm_addr = nullptr, *inv_rms_addr = nullptr, *perm_mats_addr = nullptr;
    // outputs
    ...
    aclTensor *h_pre = nullptr, *hc_before_norm = nullptr, *inv_rms = nullptr, *perm_mats = nullptr;
    aclTensor *grad_x = nullptr, *grad_phi = nullptr, *grad_alpha = nullptr, *grad_bias = nullptr;
};

// Generate all n! permutation matrices (0/1) flattened as [n!, n, n].
static std::vector<float> GenPermutationMats(int64_t n_perm, int64_t n)
{
    std::vector<float> mats(static_cast<size_t>(n_perm * n * n), 0.0f);
    ...
    do {
        for (int64_t i = 0; i < n; ++i) {
            mats[static_cast<size_t>(p_idx * n * n + i * n + perm[static_cast<size_t>(i)])] = 1.0f;
        }
        ++p_idx;
...
int CreateInputTensors(const std::vector<int64_t> &grad_hin_shape, const std::vector<int64_t> &grad_h_post_shape,
                       ...
                       const std::vector<int64_t> &perm_mats_shape, Tensors &tensors)
{
    ...
    std::vector<float> perm_mats_host_data = GenPermutationMats(perm_mats_shape[0], perm_mats_shape[1]);

    int ret = CreateAclTensorBfloat16(grad_hin_host_data, grad_hin_shape, tensors.grad_hin_addr, tensors.grad_hin);
    CHECK_RET(ret == 0, LOG_PRINT("Create grad_hin tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(grad_h_post_host_data, grad_h_post_shape, tensors.grad_h_post_addr, tensors.grad_h_post);
    CHECK_RET(ret == 0, LOG_PRINT("Create grad_h_post tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(grad_h_res_host_data, grad_h_res_shape, tensors.grad_h_res_addr, tensors.grad_h_res);
    CHECK_RET(ret == 0, LOG_PRINT("Create grad_h_res tensor failed\n"); return -1);
    ret = CreateAclTensorBfloat16(x_host_data, x_shape, tensors.x_addr, tensors.x);
    CHECK_RET(ret == 0, LOG_PRINT("Create x tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(phi_host_data, phi_shape, tensors.phi_addr, tensors.phi);
    CHECK_RET(ret == 0, LOG_PRINT("Create phi tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(alpha_host_data, alpha_shape, tensors.alpha_addr, tensors.alpha);
    CHECK_RET(ret == 0, LOG_PRINT("Create alpha tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(bias_host_data, bias_shape, tensors.bias_addr, tensors.bias);
    CHECK_RET(ret == 0, LOG_PRINT("Create bias tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(h_pre_host_data, h_pre_shape, tensors.h_pre_addr, tensors.h_pre);
    CHECK_RET(ret == 0, LOG_PRINT("Create h_pre tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(hc_before_norm_host_data, hc_before_norm_shape, tensors.hc_before_norm_addr,
                                 tensors.hc_before_norm);
    CHECK_RET(ret == 0, LOG_PRINT("Create hc_before_norm tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(inv_rms_host_data, inv_rms_shape, tensors.inv_rms_addr, tensors.inv_rms);
    CHECK_RET(ret == 0, LOG_PRINT("Create inv_rms tensor failed\n"); return -1);
    ret = CreateAclTensorFloat32(perm_mats_host_data, perm_mats_shape, tensors.perm_mats_addr, tensors.perm_mats);
    CHECK_RET(ret == 0, LOG_PRINT("Create perm_mats tensor failed\n"); return -1);
...
void DestroyTensors(Tensors &tensors)
{
    ...
    aclDestroyTensor(tensors.perm_mats);
...
void FreeDeviceMemory(Tensors &tensors)
{
    ...
    aclrtFree(tensors.perm_mats_addr);
...
int main()
{
    // Default to NPU card 14 (free/healthy on this host — card 0 was left in Alarm by an
    // earlier hung run). Override at runtime with the DEVICE_ID env var if needed.
    int32_t device_id = 14;
    const char *devEnv = std::getenv("DEVICE_ID");
    if (devEnv != nullptr) {
        int parsed = std::atoi(devEnv);
        if (parsed >= 0) {
    ...
    // Shapes — n=4, c must be a multiple of 64 (kernel constraint); CMHC uses n! perms.
    int64_t bs = 2, seq_len = 128, n = 4, c = 256;
    int64_t n_perm = 24;             // n! = 4! = 24
    int64_t hc_mix = n_perm + 2 * n; // CMHC mixing dim = 24 + 8 = 32
    double hc_eps = 1e-6;
    ...
    std::vector<int64_t> perm_mats_shape = {n_perm, n, n};
    ...
    ret = CreateInputTensors(grad_hin_shape, grad_h_post_shape, grad_h_res_shape, x_shape, phi_shape, alpha_shape,
                             bias_shape, h_pre_shape, hc_before_norm_shape, inv_rms_shape, perm_mats_shape, tensors);
    ...
    aclnnStatus aclnn_ret = aclnnMhcPreCmhcBackwardGetWorkspaceSize(
        tensors.grad_hin, tensors.grad_h_post, tensors.grad_h_res, tensors.x, tensors.phi, tensors.alpha, tensors.bias,
        tensors.h_pre, tensors.hc_before_norm, tensors.inv_rms, tensors.perm_mats, hc_eps, tensors.grad_x,
        tensors.grad_phi, tensors.grad_alpha, tensors.grad_bias, &workspace_size, &executor);
    CHECK_RET(aclnn_ret == ACL_SUCCESS,
              LOG_PRINT("aclnnMhcPreCmhcBackwardGetWorkspaceSize failed, error: %d\n", aclnn_ret); return -1);
    ...
    aclnn_ret = aclnnMhcPreCmhcBackward(workspace_addr, workspace_size, executor, stream);
    CHECK_RET(aclnn_ret == ACL_SUCCESS, LOG_PRINT("aclnnMhcPreCmhcBackward failed, error: %d\n", aclnn_ret); return -1);
    ...
    LOG_PRINT("Output tensor grad_phi (first 10 elements):\n");
    PrintTensorDataFloat(grad_phi_shape, tensors.grad_phi_addr);
