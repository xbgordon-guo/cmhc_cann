...
/*!
 * \file mhc_pre_cmhc_m_split_core_arch35.h
 * \brief
 */

#ifndef CMHC_M_SPLIT_CORE_ARCH35_H
#define CMHC_M_SPLIT_CORE_ARCH35_H
...
template <typename T>
class CmhcMSplitCorePart1 {
public:
    __aicore__ inline CmhcMSplitCorePart1()
    {
    }

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR hcFn, GM_ADDR hcScale, GM_ADDR hcBase, GM_ADDR y, GM_ADDR post,
                                GM_ADDR combFrag, const CmhcRegbaseTilingData *tilingDataPtr, TPipe *pipePtr)
    {
        pipe = pipePtr;
