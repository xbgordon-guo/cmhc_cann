                            ...
                            aclTensor *&tensor)
{
    ...
    for (int64_t i = 0; i < size; ++i) {
        // Proper float32 -> bfloat16 conversion: truncate lower 16 bits
        uint32_t int_bits = *reinterpret_cast<const uint32_t *>(&host_data[i]);
        host_data_bf16[i] = static_cast<uint16_t>(int_bits >> 16);
    }
...
int CreateAclTensorFloat32Output(const std::vector<int64_t> &shape, void *&device_addr, aclTensor *&tensor)
{
    ...
    // Initialize output to -1.0 so we can detect unwritten positions
    std::vector<float> init_data(size, -1.0f);

    aclError ret = aclrtMalloc(&device_addr, byte_size, ACL_MEM_MALLOC_HUGE_FIRST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclrtMalloc failed, error: %d\n", ret); return -1);

    ret = aclrtMemcpy(device_addr, byte_size, init_data.data(), byte_size, ACL_MEMCPY_HOST_TO_DEVICE);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclrtMemcpy init failed, error: %d\n", ret); return -1);
...
int CreateInputTensors(const std::vector<int64_t> &x_shape, const std::vector<int64_t> &phi_shape,
                       ...
                       const std::vector<int64_t> &perm_mats_shape,
                       Tensors &tensors)
{
    ...
    // perm_mats: all N! = 24 true permutation matrices, shape [n!, n, n]
    std::vector<float> perm_mats_host_data(GetShapeSize(perm_mats_shape), 0.0f);
    int64_t num_perms = perm_mats_shape[0];
    int64_t mat_dim = perm_mats_shape[1];  // mat_dim == n == 4
    // Generate all 4! = 24 permutations of [0,1,2,3]
    std::vector<int> perm(mat_dim);
    for (int i = 0; i < mat_dim; ++i) perm[i] = i;
    int p_idx = 0;
    do {
        for (int i = 0; i < mat_dim; ++i) {
            // Permutation matrix: P[i][perm[i]] = 1
            perm_mats_host_data[p_idx * mat_dim * mat_dim + i * mat_dim + perm[i]] = 1.0f;
        }
        ++p_idx;
    ...
    CHECK_RET(ret == 0, LOG_PRINT("Create perm_mats_tensor failed\n"); return -1);
    return 0;
}
...
int main()
{
    ...
    int64_t hc_mix = n_perm + 2 * n;  // hcMix = n! + 2N for CMHC variant
    ...
    ret = CreateInputTensors(x_shape, phi_shape, alpha_shape, bias_shape, perm_mats_shape, tensors);
    ...
    aclnnStatus aclnn_ret = aclnnMhcPreCmhcGetWorkspaceSize(
        tensors.x, tensors.phi, tensors.alpha, tensors.bias, tensors.perm_mats,
    ...
    LOG_PRINT("=== hIn (first 10 elements) ===\n");
    PrintTensorDataBfloat16(hin_shape, tensors.hin_addr);
    LOG_PRINT("=== hRes (first 10 elements) ===\n");
    PrintTensorDataFloat(h_res_shape, tensors.h_res_addr);
    LOG_PRINT("=== hRes[16..25] (second position, first 10 elements) ===\n");
    std::vector<int64_t> hres2_shape = {10};
    PrintTensorDataFloat(hres2_shape, (void*)((float*)tensors.h_res_addr + 16));
    LOG_PRINT("=== hRes[4096-10..4095] (last 10 elements) ===\n");
    PrintTensorDataFloat(hres2_shape, (void*)((float*)tensors.h_res_addr + 4096 - 10));
    LOG_PRINT("=== hPost (first 10 elements) ===\n");
    PrintTensorDataFloat(h_post_shape, tensors.h_post_addr);
    LOG_PRINT("=== hcBeforeNorm[0] (first 32 elements) ===\n");
    std::vector<int64_t> hcbn_shape = {32};
    PrintTensorDataFloat(hcbn_shape, tensors.hc_before_norm_addr);
    LOG_PRINT("=== bias (all 32 elements) ===\n");
    PrintTensorDataFloat(bias_shape, tensors.bias_addr);
    LOG_PRINT("=== perm_mats[0] (first 16 elements) ===\n");
    std::vector<int64_t> pm_shape = {16};
    PrintTensorDataFloat(pm_shape, tensors.perm_mats_addr);
    LOG_PRINT("=== perm_mats[23] (last perm, first 16 elements) ===\n");
    PrintTensorDataFloat(pm_shape, (void*)((float*)tensors.perm_mats_addr + 23 * 4 * 4));
