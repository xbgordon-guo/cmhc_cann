...
/*!
 * \file mhc_pre_cmhc_tiling.h
 * \brief MhcPreCmhc tiling
 */

#ifndef CMHC_TILING_H
#define CMHC_TILING_H
...
BEGIN_TILING_DATA_DEF(MhcPreCmhcTilingData)
TILING_DATA_FIELD_DEF(int64_t, bs);                       // batch size
TILING_DATA_FIELD_DEF(int64_t, hcMix);                    // nPerm + 2 * hcMult (= n! + 2N)
	TILING_DATA_FIELD_DEF(int64_t, nPerm);                    // hcMult factorial
...
REGISTER_TILING_DATA_CLASS(MhcPreCmhc, MhcPreCmhcTilingData)


// Regbase TilingData
BEGIN_TILING_DATA_DEF(CmhcRegbaseTilingData)
...
REGISTER_TILING_DATA_CLASS(Cmhc_1000, CmhcRegbaseTilingData)
REGISTER_TILING_DATA_CLASS(Cmhc_1001, CmhcRegbaseTilingData)
REGISTER_TILING_DATA_CLASS(Cmhc_2000, CmhcRegbaseTilingData)
REGISTER_TILING_DATA_CLASS(Cmhc_2001, CmhcRegbaseTilingData)

struct CmhcCompileInfo {
    uint64_t coreNum = 0;
    uint64_t ubSize = 0;
};
...
class CmhcTiling {
public:
    explicit CmhcTiling(gert::TilingContext* tilingContext) : context_(tilingContext) {}
    ~CmhcTiling() = default;
...
private:
    ...
    MhcPreCmhcTilingData tilingData_;
...
class CmhcTilingRegbase {
public:
    explicit CmhcTilingRegbase(gert::TilingContext *tilingContext);
    ~CmhcTilingRegbase() = default;
...
private:
    gert::TilingContext *context_ = nullptr;
    uint64_t tilingKey_ = 0;
    CmhcRegbaseTilingData tilingData_;
    uint64_t aivCoreNum_ = 0;
    uint64_t aicCoreNum_ = 0;
    uint64_t workspaceSize_ = 0;
