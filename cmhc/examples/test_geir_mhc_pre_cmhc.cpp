...
#include "../op_graph/mhc_pre_cmhc_proto.h"
...
void SetOutputDesc(ge::op::MhcPreCmhc &op)
{
...
int CreateGraph(std::vector<Tensor> &input_tensors, std::vector<Operator> &input_ops, std::vector<Operator> &outputs,
                Graph &graph)
{
    auto cmhc = ge::op::MhcPreCmhc("test_geir_mhc_pre_cmhc");
    ...
    if (AddDataInput("alpha", 2, ge::DT_FLOAT, {3}, graph, input_tensors, input_ops, cmhc) != SUCCESS) {
        return FAILED;
    }
    if (AddDataInput("bias", 3, ge::DT_FLOAT, {24}, graph, input_tensors, input_ops, cmhc) != SUCCESS) {
        return FAILED;
    }

    cmhc.set_attr_hc_mult(4);
    cmhc.set_attr_num_iters(20);
    cmhc.set_attr_hc_eps(1e-6f);
    cmhc.set_attr_norm_eps(1e-6f);
    cmhc.set_attr_need_backward(true);
    SetOutputDesc(cmhc);

    outputs.push_back(cmhc);
    return SUCCESS;
}

int main()
{
    Graph graph("mhc_pre_cmhc_geir_graph");
