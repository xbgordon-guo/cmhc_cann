...
class CmhcStage1 {
public:
    __aicore__ inline CmhcStage1() {}

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR phi, GM_ADDR invRms, GM_ADDR hcBeforeNorm,
                                GM_ADDR workspace, const CmhcTilingData *tilingDataPtr, TPipe *pipePtr)
    {
        pipe = pipePtr;
...
public:
    TPipe *pipe;
    const CmhcTilingData *tilingData;
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR hcScale, GM_ADDR hcBase, GM_ADDR y,
                                ...
                                GM_ADDR workspace,
                                const CmhcTilingData *tilingDataPtr, TPipe *pipePtr)
    {
        pipe = pipePtr;
        tilingData = tilingDataPtr;
        needGrad_ = tilingData->needGrad;
    ...
    __aicore__ inline void Process(bool isTailBsLoop = false)
    {
        ...
        if ASCEND_IS_AIV {
            ...
            for (int64_t rowOuterIdx = 0; rowOuterIdx < rowOuterLoop; rowOuterIdx++) {
                ...
                // === h_res: Softmax-permutation (replaces Sinkhorn-Knopp) ===
                // Load permutation logits: hcBeforeNorm[2N : 2N+nPerm]
                ...
                // Apply r_inv * alpha_res + bias_res
                combFragLocal = combFragQue.AllocTensor<float>();
                {
                    float aRes = hcScaleGm.GetValue(NUM_TWO);
                    for (int64_t j = 0; j < curRowFactor; ++j) {
                        float rms = squareSumOutLocal.GetValue(j);
                        for (int64_t k = 0; k < nPerm; ++k) {
                            float v = mixes2Local.GetValue(j * nPerm + k) * rms * aRes
                                      + hcBase2Local.GetValue(k);
                            combFragLocal.SetValue(j * nPerm + k, v);
                        }
                    }
                }
                ...
                // Softmax over permutation logits
                for (int64_t r = 0; r < curRowFactor; ++r) {
                    float mx = combFragLocal.GetValue(r * nPerm);
                    for (int64_t k = 1; k < nPerm; ++k) {
                        float v = combFragLocal.GetValue(r * nPerm + k);
                        if (v > mx) mx = v;
                    }
                    for (int64_t k = 0; k < nPerm; ++k)
                        combFragLocal.SetValue(r * nPerm + k, combFragLocal.GetValue(r * nPerm + k) - mx);
                }
                PipeBarrier<PIPE_V>();
                Exp(combFragLocal, combFragLocal, curRowFactor * nPerm);
                PipeBarrier<PIPE_V>();

                for (int64_t r = 0; r < curRowFactor; ++r) {
                    float es = 0;
                    for (int64_t k = 0; k < nPerm; ++k) es += combFragLocal.GetValue(r * nPerm + k);
                    float ies = (es > 0) ? (1.0f / es) : 1.0f;
                    for (int64_t k = 0; k < nPerm; ++k)
                        combFragLocal.SetValue(r * nPerm + k, combFragLocal.GetValue(r * nPerm + k) * ies);
                }
                PipeBarrier<PIPE_V>();
                ...
                // Birkhoff-von Neumann: permutation weighted sum
                {
                    ...
                    for (int64_t r = 0; r < curRowFactor; ++r) {
                        for (int64_t i = 0; i < nn; ++i)
                            reduceLocal.SetValue(r * nn + i, 0.0f);
                    }
                    for (int64_t r = 0; r < curRowFactor; ++r) {
                ...
                // Copy permutation result to combFragLocal for output as h_res
                {
                    int64_t nn = tilingData->hcMult * tilingData->hcMult;
                    for (int64_t r = 0; r < curRowFactor; ++r) {
                        for (int64_t i = 0; i < nn; ++i) {
                            int64_t off = r * nn + i;
                            combFragLocal.SetValue(off, reduceLocal.GetValue(off));
                        }
                    }
                }
                PipeBarrier<PIPE_V>();
