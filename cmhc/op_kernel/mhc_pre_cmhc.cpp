...
#include "mhc_pre_cmhc_m_k_split_core.h"
#include "mhc_pre_cmhc_m_split_core.h"
#include "mhc_pre_cmhc_base.h"
using namespace MhcPreCmhc;
                                             ...
                                             GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    GET_TILING_DATA_WITH_STRUCT(MhcPreCmhcTilingData, tiling_data_in, tiling);
    const MhcPreCmhcTilingData *__restrict tilingData = &tiling_data_in;

    if (TILING_KEY_IS(0)) {
        // -----------split m--------------
        TPipe pipe;
        MhcPreCmhc::MhcPreCmhcStage1<DTYPE_X> op1;
        ...
        for (int64_t bsLoopIdx = 0; bsLoopIdx < tilingData->bsLoop; bsLoopIdx++) {
            ...
            TPipe pipe;
            MhcPreCmhc::MhcPreCmhcMembaseKSplitCorePart1<DTYPE_X> op;
            op.Init(x, phi, userWs, tilingData, &pipe, curBsOffset, curBs, isTailBsLoop);
            op.Process(curBsOffset, curBs, isTailBsLoop);
            pipe.Destroy();

            TPipe pipeStage2;
            MhcPreCmhc::MhcPreCmhcMembaseKSplitCorePart2<DTYPE_X> op2;
            op2.Init(x, alpha, bias, hin, hPost, hRes,
                        hPre, hcBeforeNorm, invRms, sumOut, normOut,
