...
OP_TYPE_REGISTER(Cmhc);

static const aclTensor *CmhcAiCore(const aclTensor *x, const aclTensor *phi, const aclTensor *alpha,
...
{
    L0_DFX(CmhcAiCore, x, phi, alpha, bias, hcMult, numIters, hcEps, normEps, needBackward,
    ...
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Cmhc ADD_TO_LAUNCHER_LIST_AICORE failed."),
             return nullptr);
    return hin;
}

const aclTensor *Cmhc(const aclTensor *x, const aclTensor *phi, const aclTensor *alpha,
                                const aclTensor *bias, int64_t hcMult, int64_t numIters, double hcEps,
                                double normEps, bool needBackward,
                                aclTensor *hin, aclTensor *hPost, aclTensor *hRes,
                                aclTensor *hPre, aclTensor *hcBeforeNorm, aclTensor *invRms,
                                ...
                                aclOpExecutor *executor)
{
    return CmhcAiCore(x, phi, alpha, bias, hcMult, numIters, hcEps, normEps, needBackward,
