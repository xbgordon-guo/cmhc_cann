    ...
    
{
    ...
    if (x->IsEmpty() || hin->IsEmpty() || hPost->IsEmpty() || hRes->IsEmpty()) {
        OP_LOGW("[aclnnCmhc] Input tensor is empty, skip computation and return success.");
        *workspaceSize = 0;
    ...
    const aclTensor *kernelOut = l0op::Cmhc(
...
aclnnStatus aclnnCmhc(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnCmhc);
