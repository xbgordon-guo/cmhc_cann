 ...
 */
 ...
 /*!
 * \file aclnn_mhc_pre_cmhc.cpp
 * \brief aclnn_mhc_pre_cmhc
 */

#include "aclnn_mhc_pre_cmhc.h"
#include "mhc_pre_cmhc.h"
    ...
    
{
    ...
    if (x->IsEmpty() || hin->IsEmpty() || hPost->IsEmpty() || hRes->IsEmpty()) {
        OP_LOGW("[aclnnMhcPreCmhc] Input tensor is empty, skip computation and return success.");
        *workspaceSize = 0;
...
aclnnStatus aclnnMhcPreCmhc(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnMhcPreCmhc);
